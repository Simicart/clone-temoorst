...
const OrderTotal = (props) => {
    ...
    function renderSubToTal() {
        return(
            <CardItem style={{ flexDirection: 'row' }}>
                <Text style={[styles.title, {color: '#595656', flex: 4, textAlign: 'left'}]}>{Identify.__('Sub Total')}</Text>
    ...
    function renderDeliveryCharge() {
        return(
            <CardItem style={{ flexDirection: 'row' }}>
                <Text style={[styles.title, {color: '#595656', flex: 4, textAlign: 'left'}]}>{Identify.__('Delivery Charge')}</Text>
    ...
    function renderGrandTotal() {
        return(
            <CardItem style={{ flexDirection: 'row' }}>
                <Text style={[styles.title, { flex: 4, fontWeight: 'bold', fontSize: 18, textAlign: 'left' }]}>{Identify.__('Grand Total')}</Text>
                <Text style={{ flex: 6, fontWeight: 'bold', fontSize: 18, textAlign: 'right' }}>{Identify.formatPriceWithCurrency(total.grand_total_incl_tax, total.currency_symbol)}</Text>
            </CardItem>
        )
    }
