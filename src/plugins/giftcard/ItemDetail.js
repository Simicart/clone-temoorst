...
class ItemDetail extends SimiPageComponent {
    ...
    setData(data) {
        ...
        if (data.errors) {
            ...
            Toast.show({ text: text, duration: 1500 , textStyle: {fontFamily: material.fontFamily}});
        } else {
            if (this.remove === 1 && data.simicustomercredit.message) {
                Toast.show({ text: data.simicustomercredit.message, duration: 2500, textStyle: {fontFamily: material.fontFamily} });
                return;
            }
            if (this.sendemail === 1 && data.message.success) {
                Toast.show({ text: data.message.success, duration: 2500, textStyle: {fontFamily: material.fontFamily} });
                this.setState({
                    send_email: false
                });
                this.sendemail = 0;
