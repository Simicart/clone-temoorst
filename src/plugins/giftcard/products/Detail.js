...
class GiftCardProductDetail extends SimiPageComponent{
    ...
    renderPhoneLayout(){
        if(this.state.data){
            ...
            let review = this.dispatchPluginGiftCard(data,'giftcard_review');
            return(
                <Container>
    ...
    dispatchPluginGiftCard(data,keyPlugin){
        let plugins = [];
    ...
    getOptionParams = () => {
        ...
        if(this.send_friend){
            ...
            if(this.recipient_name){
                ...
                Toast.show({text: 'Please Enter Recipient Name !',textStyle: {fontFamily: material.fontFamily}});
                return;
            }
            if(!this.recipient_email){
                Toast.show({text: 'Please Enter Recipient Email !',textStyle: {fontFamily: material.fontFamily}});
                return;
            }else{
                let email = this.recipient_email;
                if (!this.validateEmail(email))
                {
                    Toast.show({text: 'Recipient Email Invalid !',textStyle: {fontFamily: material.fontFamily}});
                    return;
                }else {
            ...
            if(this.message){
                ...
                if(msg.length >= max){
                    Toast.show({text: `Message max length (${max}) `,textStyle: {fontFamily: material.fontFamily}});
                    return;
                }else {
                    json['message'] = msg;
                }
