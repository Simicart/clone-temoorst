...
class ApplyGiftCode extends SimiComponent{
    ...
    setData(data){
        ...
        if(data.errors){
            ...
            Toast.show({text: text, textStyle : {fontFamily: material.fontFamily}})
        }else {
    ...
    handleUseGiftCode = (type = 1)=>{
        ...
        if(type === 1){
            if (this.state.existed_giftcode === 0 && this.state.giftcode === 0 ){
                Toast.show({
                    test: Identify.__('Please enter Gift Code !') ,
    ...
    renderHeader(){
        return (
            <View
                style={{paddingTop: 20, paddingBottom: 20, borderTopWidth: 0.3, borderColor: '#c9c9c9'}}
            >
                <Text style={{fontWeight: '900', fontSize: 16}}>{Identify.__('GIFT CARD')}</Text>
            </View>
        )
    }
        ...
        return(
            <View>
                <TouchableOpacity
                    ...
                    onPress={() => this.updateCheck(1)}>
                    <Icon name={this.state.use_credit ? 'ios-checkmark-circle-outline' : 'ios-radio-button-off'} />
                    <Text style={{flexGrow: 1, marginRight: 7, marginLeft: 7}}>{Identify.__('Use Gift Card credit to check out')} ({data.customer.balance})</Text>
                </TouchableOpacity>
        ...
        return(
            <View>
                <TouchableOpacity
                    ...
                    onPress={() => this.updateCheck(2)}>
                    <Icon name={this.state.use_giftcode ? 'ios-checkmark-circle-outline' : 'ios-radio-button-off'} />
                    <Text style={{flexGrow: 1, marginRight: 7, marginLeft: 7}}>{Identify.__('Use Gift Card to check out')}</Text>
                </TouchableOpacity>
    ...
    renderPhoneLayout(){
        ...
        if(data.hasOwnProperty('gift_card') && data.gift_card.hasOwnProperty('customer')){
            if(data.gift_card.use_giftcard){
                if(this.state.useGiftCard){
                    return(
                        <View
                            style={{
                                marginLeft: 15,
                ...
                return (
                    <TouchableOpacity
                        onPress={() => {
                            this.setState({useGiftCard: true})
                        }}
                        style={{
                            flexDirection: 'row',
                            marginRight: 15,
                    ...
                    >
                        <Icon style={{fontSize: 16, color: Identify.theme.content_color}} name={'md-add'}/>
                        <Text style={{marginLeft: 7, marginRight: 7, fontSize: 16}}>{Identify.__('Use gift card for checkout').toUpperCase()}</Text>
                    </TouchableOpacity>
                )
            }
            return <View
