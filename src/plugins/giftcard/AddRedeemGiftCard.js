...
class AddRedeemGiftCard extends SimiPageComponent{
    ...
    setData(data){
        if (data.errors) {
            ...
            Toast.show({text : text, textStyle: {fontFamily: material.fontFamily}, duration: 3000});
        } else {
            Toast.show({text : data.message.success, textStyle: {fontFamily: material.fontFamily}, duration: 3000});
    ...
    addAction = (type = 1) => {
        ...
        if(!giftcode || giftcode === ''){
            Toast.show({text: Identify.__('Please enter a Gift Code !'), textStyle: {fontFamily: material.fontFamily}, duration: 3000});
            return;
        }
        let json =  {
            giftcode
